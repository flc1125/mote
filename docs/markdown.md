...
**Current Viewer source (after #55):** protocol-relative image URLs such as `//example.com/image.png`, including slash/backslash variants, are rejected in Markdown images and HTML `src`/`srcset`. Use an explicit URL such as `https://example.com/image.png`. This correction is not included in the v0.6.0 Viewer source; self-hosted instances need the updated Worker. Normal relative asset references and same-origin asset URLs remain supported.
