...
| 路由                                   | 说明                                                                                   |
| -------------------------------------- | -------------------------------------------------------------------------------------- |
| `GET/HEAD /{document-id}`              | 渲染后的 HTML；HEAD 仅返回响应头（HTML 白名单净化，仅可信目录/代码复制脚本，严格 CSP） |
...
- 安全头：`Content-Security-Policy`（`script-src` 仅授权固定目录与代码复制脚本的 SHA-256 哈希）、`Referrer-Policy: no-referrer`、`X-Robots-Tag: noindex` 等，详见 [security.md](security.md)。
