...
| 路由                              | 说明                                                             |
| --------------------------------- | ---------------------------------------------------------------- |
| `GET /{document-id}`              | 渲染后的 HTML（HTML 白名单净化，仅可信目录和历史脚本，严格 CSP） |
...
- 安全头：`Content-Security-Policy`（`script-src` 仅授权固定目录和历史脚本的 SHA-256 哈希）、`Referrer-Policy: no-referrer`、`X-Robots-Tag: noindex` 等，详见 [security.md](security.md)。
