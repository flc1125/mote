...
| 路由                                   | 说明                                                                                                      |
| -------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `GET/HEAD /{document-id}`              | 渲染后的 HTML；HEAD 仅返回响应头（HTML 白名单净化，仅可信目录与折叠定位/代码复制/图片查看脚本，严格 CSP） |
...
- 安全头：`Content-Security-Policy`（`script-src` 仅授权固定导航（含目录、折叠定位和标签切换）、代码复制及图片查看脚本的 SHA-256 哈希）、`Referrer-Policy: no-referrer`、`X-Robots-Tag: noindex` 等，详见 [security.md](security.md)。
