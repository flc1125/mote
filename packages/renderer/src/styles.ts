...
article details.markdown-alert {
  padding: 0.85em 1.1em;
  border: 0;
  border-left: 3px solid var(--alert-color);
  border-radius: 0 8px 8px 0;
}
/* Keep deeply nested content usable on narrow screens. */
.markdown-alert .markdown-alert { padding-inline: 0.65em; }
article details.markdown-alert > summary.markdown-alert-title {
  margin: 0;
  list-style: none;
}
article details.markdown-alert[open] > summary { margin-bottom: 0.45em; }
.markdown-alert > summary::-webkit-details-marker { display: none; }
.markdown-alert > summary::marker { content: ''; }
.markdown-alert > summary::after {
  content: '';
  flex: 0 0 0.4em;
  height: 0.4em;
...
.markdown-alert[open] > summary::after { transform: rotate(45deg); }
.markdown-alert > summary:focus-visible { outline: 2px solid var(--alert-color); outline-offset: 5px; }
@media print {
  details.markdown-alert::details-content { display: contents; content-visibility: visible; }
  .markdown-alert > summary::after { display: none; }
  .markdown-alert { print-color-adjust: exact; }
}
