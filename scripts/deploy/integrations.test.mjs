...
describe('workflow trust boundaries', () => {
  it('provides only a manual server entry with caller-owned concurrency', async () => {
    ...
    expect(shared).not.toMatch(/\$\{\{\s*secrets\.|^\s*secrets:|^ {4}environment:/m);
    expect(shared).not.toMatch(/action\.mjs (deploy|write|install-cli)/);
    const deploy = caller.split('  deploy:')[1].split('  write-smoke:')[0];
    ...
    const write = caller.split('  write-smoke:')[1];
    expect(write).toContain('needs: [prepare, deploy]');
    expect(write).not.toContain('CLOUDFLARE_API_TOKEN');
    expect(caller.match(/secrets\.CLOUDFLARE_API_TOKEN/g)).toHaveLength(1);
    expect(caller + shared).not.toContain('persist-credentials: true');
  });
});
