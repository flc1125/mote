...
两个 Worker 通过 Cloudflare Routes 共用 `mote.flc.io`：API Worker 绑定 `mote.flc.io/api/*`，Viewer 绑定 `mote.flc.io/*`，Cloudflare 按最具体路由优先匹配，DNS/TLS 自动处理；R2 通过 Worker binding 直接访问，因此无需额外服务器。

注意：`mote.flc.io` 不得使用 Custom Domain 绑定 Worker，因为 Custom Domain 会覆盖同主机名的 `api/*` 路由。
...
```text
MOTE_API_URL=https://mote.flc.io
```
...
```bash
wrangler secret put MOTE_TOKEN
```
...
正式环境使用 Routes（路径路由），不使用 Custom Domain。
...
- [ ] 设置 Secret
- [ ] 绑定 R2
- [ ] 配置路由 `mote.flc.io/*` → mote-viewer
- [ ] 配置路由 `mote.flc.io/api/*` → mote-api
- [ ] 启用 Workers Cache
- [ ] 验证 Cache HIT
